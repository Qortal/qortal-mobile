...
export const useFetchResources = () => {
  ...
  const setGlobalDownloads = useSetRecoilState(globalDownloadsAtom);
  ...
  const startGlobalDownload = useCallback(
    async (
      ...
      const existingValues = await getResourceStatus(resourceId) as any;
      if (existingValues && existingValues?.isFetching) return;

      const intervalMap: Record<string, NodeJS.Timeout | undefined> = {};
      const timeoutMap: Record<string, NodeJS.Timeout | undefined> = {};
      const retryTimeoutMap: Record<string, NodeJS.Timeout | undefined> = {};
      ...
      const callFunction = async (build?: boolean, isRecalling?: boolean) => {
        try {
          ...
          const currentStatus = await getResourceStatus(resourceId) as any;
          ...
          if (!build) {
            ...
            fetch(
              ...
              .then(async (peersData) => {
                ...
                const peers: any[] = peersData?.peers ?? [];
                const currentStatus = await getResourceStatus(resourceId) as any;
  ...
  const downloadResource = useCallback(
    async (
      { service, name, identifier }: { service: string; name: string; identifier: string },
      build?: boolean,
